...
echo "🧪 Testing Chaos Arena scaffold locally..."

# Create test directory
TEST_DIR="chaos-arena-test"
rm -rf "$TEST_DIR"
mkdir -p "$TEST_DIR"/{src,include,.github/workflows}
cd "$TEST_DIR"
...
struct Ring {
  ...
  T get(size_t i) const { size_t h=head.load(); size_t n=size(); size_t s=(h>=N? h%N:0); return buf[(s+i)%N]; }
};
...
# Build with DEV_BUILD enabled
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-DDEV_BUILD"
cmake --build build --config Release --parallel

echo "🧪 Testing scaffold..."
./build/chaos

echo "✅ Scaffold test successful!"
echo "📁 Test files created in: $TEST_DIR"
echo "🚀 Ready to bootstrap with GitHub!"